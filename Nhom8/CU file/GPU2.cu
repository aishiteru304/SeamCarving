...
__global__ void Find_Min_Index(double* data, int* index, int n)
{
            ...
            if (threadIdx.x < stride) {
                if(threadIdx.x+stride<n)
                {
                  if(sdata[threadIdx.x] > sdata[threadIdx.x + stride])
                  {
                      ...
                      index[threadIdx.x] = index[threadIdx.x+stride];
                  }
                }
            }
            __syncthreads();
...
__host__ int FindIndexOfMin(double* a, int left, int right)
{
    int n = right-left+1 ;
    double* device_data;
    CHECK(cudaMalloc(&device_data,  sizeof(double)*n));
    CHECK(cudaMemcpy(device_data, a+left, sizeof(double)*n, cudaMemcpyHostToDevice));
    int* device_index;
    CHECK(cudaMalloc(&device_index, sizeof(int)*n));
    Find_Min_Index<<<dim3(1), dim3(1024)>>>(device_data, device_index, n);
    CHECK(cudaGetLastError());
    CHECK(cudaPeekAtLastError());
    int min_index;
    cudaMemcpy(&min_index, device_index, sizeof(int), cudaMemcpyDeviceToHost);
    return min_index + left;
}
__device__ int Device_FindIndexOfMin(double* a, int left, int right)
{
...
__global__ void Calculate_Cumulative(double* energies,  int width, int height, double* cumulative, int* direction)
{
  ...
  extern __shared__ double s_cumulative[];
  if (idx < width)
  {
      s_cumulative[idx] = energies[idx];
  ...
  for(int j = 1; j < height; j++)
  {
      ...
      if(idx == 0)
      {
          a[0] = 1.0/0.0; a[1]=s_cumulative[idx]; a[2]=s_cumulative[idx+1];
      }
      else
      {
          if(idx == width -1)
          {
              a[0] = s_cumulative[idx-1]; a[1]=s_cumulative[idx];a[2]=1.0/0.0;
          }
          else
        {
             a[0] = s_cumulative[idx-1]; a[1]=s_cumulative[idx];a[2]=s_cumulative[idx+1];
        }
      }
      direction_val = Device_FindIndexOfMin(a, 0, 2);
      direction[j*width+idx] = direction_val;
      s_cumulative[idx] = s_cumulative[idx+direction_val-1]+energies[j*width+idx];
      cumulative[j*width+idx] = s_cumulative[idx];
      __syncthreads();
  }
...
static int*  get_route_of_minimum_cumulative_enery(const double* energies, int width, int height) {
    ...
    Calculate_Cumulative<<<gridSize, blockSize, min(1024, width)*sizeof(double)>>>(device_energies, width, height, device_cumulative, device_direction);
    cudaDeviceSynchronize();
